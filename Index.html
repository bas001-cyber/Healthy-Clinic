...
<body class="bg-slate-100 text-slate-800">
    ...
    <script type="text/babel">
        ...
        // 🟢 [จุดแก้ไขที่ 1] วางค่า Config จาก Firebase ตรงนี้ (ถ้าไม่มี ระบบจะทำงานแบบ Offline)
        const MY_FIREBASE_CONFIG = {
            apiKey: "YOUR_API_KEY", // <--- เอาค่าจาก Firebase Console มาแทนที่ตรงนี้
        ...
        // --- Mock Data ---
        const DOCTORS = [
            { id: 1, name: 'นพ. สมชาย รักดี', department: 'อายุรกรรม (General Medicine)', image: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Somchai&backgroundColor=b6e3f4' },
            { id: 2, name: 'พญ. วิไล ใจดี', department: 'กุมารเวช (Pediatrics)', image: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Wilai&backgroundColor=ffdfbf' },
            { id: 3, name: 'ทพ. เอก ฟันสวย', department: 'ทันตกรรม (Dentistry)', image: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Ek&backgroundColor=c0aede' },
        ];
        ...
        const NotificationToast = ({ message, type, onClose }) => (
            <div className={`fixed top-4 right-4 z-50 p-4 rounded-lg shadow-lg flex items-center gap-3 animate-fade-in-down ${
                ...
                <div>
                    <p className="font-bold text-sm">{type === 'success' ? 'สำเร็จ' : type === 'warning' ? 'แจ้งเตือนระบบ' : 'ข้อผิดพลาด'}</p>
                ...
                <div className="animate-fade-in">
                    <div className="bg-white p-6 rounded-2xl shadow-sm mb-6 border border-slate-100">
                        <h2 className="text-2xl font-bold text-teal-800 mb-4 flex items-center gap-2">
                            <i className="fa-solid fa-user-injured"></i> ลงนัดหมายแพทย์ (Patient Booking)
                        </h2>
                        <div className="grid md:grid-cols-2 gap-8">
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-sm font-medium text-slate-600 mb-1">เลือกแพทย์</label>
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-slate-600 mb-1">เวลาที่ว่าง</label>
                                    <div className="grid grid-cols-2 gap-2">
                                        {loading ? (
                                            <div className="col-span-2 text-center py-4 text-slate-400">กำลังบันทึกข้อมูล...</div>
                                        ) : TIME_SLOTS.map(time => {
                            ...
                            <div className="bg-slate-50 p-6 rounded-xl border border-slate-200 h-fit">
                                <h3 className="font-semibold text-slate-800 mb-4 flex items-center gap-2"><i className="fa-solid fa-file-lines"></i> ข้อมูลผู้ป่วย</h3>
                                <div className="space-y-3">
                                    <div>
                                        <label className="text-xs font-bold text-slate-500 uppercase">ชื่อ-นามสกุล <span className="text-red-500">*</span></label>
                                        <input type="text" className="w-full p-2.5 rounded-lg border border-slate-300 mt-1 focus:ring-2 focus:ring-teal-500 outline-none" placeholder="เช่น สมชาย รักดี" value={patientForm.name} onChange={(e) => setPatientForm({...patientForm, name: e.target.value})} />
                                    </div>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                        <div>
                                            <label className="text-xs font-bold text-slate-500 uppercase">เบอร์โทรศัพท์ <span className="text-red-500">*</span></label>
                                    ...
                                    <div>
                                        <label className="text-xs font-bold text-slate-500 uppercase">อาการเบื้องต้น</label>
                                        <textarea className="w-full p-2.5 rounded-lg border border-slate-300 mt-1 focus:ring-2 focus:ring-teal-500 outline-none" rows="3" placeholder="เช่น มีไข้, ไอ, ปวดฟัน..." value={patientForm.symptom} onChange={(e) => setPatientForm({...patientForm, symptom: e.target.value})}></textarea>
                                    </div>
                                    <div className="pt-4">
                                        <button onClick={handleBooking} disabled={loading} className={`w-full py-3 bg-teal-600 hover:bg-teal-700 text-white rounded-xl font-bold shadow-lg shadow-teal-200 transition-all active:scale-95 flex items-center justify-center gap-2 ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}>
                                            {loading ? 'กำลังบันทึก...' : <span>ยืนยันการจอง <i className="fa-solid fa-chevron-right"></i></span>}
                                        </button>
                                    </div>
                                </div>
                            </div>
            ...
            return (
                <div className="animate-fade-in">
                    <div className="flex justify-between items-center mb-6">
                        <h2 className="text-2xl font-bold text-teal-800 flex items-center gap-2"><i className="fa-solid fa-shield-halved"></i> Dashboard เจ้าหน้าที่</h2>
                        <div className="flex gap-3 items-center">
                            ...
                            <button onClick={openBackupModal} className="px-4 py-2 bg-teal-100 text-teal-700 rounded-lg hover:bg-teal-200 font-medium text-sm transition-colors flex items-center gap-2"><i className="fa-solid fa-database"></i> จัดการข้อมูล</button>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-teal-50 text-teal-800 border-b border-teal-100">
                                    ...
                                    <th className="p-4 font-semibold">อาการ</th>
                            ...
                            <tbody>
                                {filteredApps.length === 0 ? (
                                    <tr><td colSpan="6" className="p-8 text-center text-slate-400">ไม่มีนัดหมายในวัน/ช่วงเวลานี้ (Offline Mode: ข้อมูลอาจไม่ครบถ้วน)</td></tr>
                                ) : (
                                        ...
                                        <tr key={app.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                                            ...
                                            <td className="p-4 text-right">
                                                <div className="flex justify-end gap-2">
                                                    <button onClick={() => handleSendEmail(app)} className={`px-3 py-1 rounded-lg text-sm transition-colors border flex items-center gap-1 ${!app.email || app.email === '-' ? 'text-slate-400 border-slate-200 cursor-not-allowed' : 'text-indigo-600 hover:bg-indigo-50 border-indigo-200'}`} title="ส่งอีเมล" disabled={!app.email || app.email === '-'}><i className="fa-solid fa-envelope"></i></button>
                                                    <button onClick={() => handleSendNotification(app)} className="text-blue-600 hover:bg-blue-50 px-3 py-1 rounded-lg text-sm transition-colors border border-blue-200 flex items-center gap-1" title="ส่งแจ้งเตือน"><i className="fa-solid fa-bell"></i></button>
                                                    <button onClick={() => handleCancelAppointment(app.id)} className="text-red-500 hover:bg-red-50 px-3 py-1 rounded-lg text-sm transition-colors border border-red-200" title="ยกเลิก"><i className="fa-solid fa-trash"></i></button>
                                                </div>
                                            </td>
                                        </tr>
                                    ))
        ...
        function HealthyClinicApp() {
            ...
            // --- [จุดแก้ไขที่ 2] ระบบตรวจจับโหมด Online/Offline ---
            useEffect(() => {
                ...
                if (fbParams) { // Online Mode
                    ...
                    setAppointments([
                        { id: 1, patientName: 'สมศรี มีสุข (Offline)', email: 'somsri@email.com', doctorId: 1, date: '2025-11-24', time: '09:00 - 09:30', phone: '081-111-1111', symptom: 'ปวดหัว ตัวร้อน', status: 'confirmed' },
                        { id: 2, patientName: 'เด็กชาย กล้า (Offline)', email: 'mom_kla@email.com', doctorId: 2, date: '2025-11-24', time: '10:00 - 10:30', phone: '089-999-9999', symptom: 'ฉีดวัคซีนไข้หวัดใหญ่', status: 'confirmed' },
                    ]);
                }
            }, []);
            ...
            const handleLogin = () => {
                if (loginCredentials.username === 'admin' && loginCredentials.password === '1234') {
                    ...
                    setView('staff');
                    showNotification('เข้าสู่ระบบเจ้าหน้าที่สำเร็จ', 'success');
                    setLoginCredentials({ username: '', password: '' });
                } else {
                    showNotification('ชื่อผู้ใช้หรือรหัสผ่านไม่ถูกต้อง', 'error');
                }
            };
            const handleLogout = () => { setIsLoggedIn(false); setView('home'); showNotification('ออกจากระบบเรียบร้อยแล้ว', 'success'); };
            ...
            const handleBooking = async () => {
                if (!patientForm.name || !patientForm.phone || !selectedTime) return showNotification('กรุณากรอกชื่อ เบอร์โทร และเลือกเวลา', 'error');
                if (isSlotTaken(selectedDoctor, selectedDate, selectedTime)) return showNotification('เวลานี้ถูกจองแล้ว กรุณาเลือกเวลาอื่น', 'error');
                ...
                if (fbParams && fbParams.db && fbParams.appId) {
                    ...
                    try {
                        ...
                        showNotification('เกิดข้อผิดพลาดในการเชื่อมต่อฐานข้อมูล: ' + error.message, 'error');
                    }
                } else {
            ...
            const showSuccess = (isOffline = false) => {
                ...
                const msg = isOffline ? 'บันทึกในเครื่องสำเร็จ (Offline Mode)' : 'บันทึกนัดหมายสำเร็จ';
                if (patientForm.email && patientForm.email !== '-') {
                    setTimeout(() => showNotification(`📧 ระบบส่งอีเมลยืนยันอัตโนมัติไปยัง ${patientForm.email} แล้ว`, 'success'), 800);
                } else {
            ...
            const handleSendNotification = (appointment) => showNotification(`ส่งข้อความแจ้งเตือน (SMS/LINE) ไปยัง คุณ${appointment.patientName} เรียบร้อยแล้ว`, 'success');
            const handleSendEmail = (appointment) => {
                if (!appointment.email || appointment.email === '-') return showNotification(`ไม่พบข้อมูลอีเมลของผู้ป่วยรายนี้`, 'error');
                showNotification(`ส่งอีเมลยืนยันนัดหมายไปยัง ${appointment.email} เรียบร้อยแล้ว`, 'success');
            };
                ...
                if (confirm('ยืนยันการยกเลิกนัดหมายนี้?')) {
                    ...
                    if (fbParams && fbParams.db) {
                        ...
                        try {
                            ...
                            showNotification('ยกเลิกนัดหมายเรียบร้อยแล้ว');
                        } catch(e) { showNotification('ไม่สามารถยกเลิกได้ (DB Error)', 'error'); }
                    } else {
                        // Offline delete
                        setAppointments(prev => prev.filter(app => app.id !== id));
                        showNotification('ลบนัดหมายออกจากเครื่องเรียบร้อยแล้ว', 'success');
                    }
                }
            };
            ...
            return (
                <div className="min-h-screen font-sans text-slate-800">
                    ...
                    <Modal isOpen={showLoginModal} onClose={() => setShowLoginModal(false)} title="เข้าสู่ระบบเจ้าหน้าที่">
                        <div className="space-y-4">
                            ...
                            <div><label className="block text-sm font-medium text-slate-600 mb-1">ชื่อผู้ใช้</label><input type="text" className="w-full p-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-teal-500 outline-none" placeholder="ระบุชื่อผู้ใช้" value={loginCredentials.username} onChange={(e) => setLoginCredentials({...loginCredentials, username: e.target.value})} /></div>
                            <div><label className="block text-sm font-medium text-slate-600 mb-1">รหัสผ่าน</label><input type="password" className="w-full p-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-teal-500 outline-none" placeholder="ระบุรหัสผ่าน" value={loginCredentials.password} onChange={(e) => setLoginCredentials({...loginCredentials, password: e.target.value})} /></div>
                            <button onClick={handleLogin} className="w-full bg-teal-600 text-white py-2.5 rounded-lg hover:bg-teal-700 font-bold mt-2">เข้าสู่ระบบ</button>
                        </div>
                    </Modal>
                    <Modal isOpen={showBackupModal} onClose={() => setShowBackupModal(false)} title="จัดการข้อมูลระบบ">
                        <div className="space-y-6 text-center">
                            ...
                            <button onClick={downloadBackup} className="w-full bg-white border border-teal-200 text-teal-700 hover:bg-teal-50 py-2 rounded-lg text-sm font-medium transition-colors">ดาวน์โหลดไฟล์ Backup</button>
                            <p className="text-xs text-slate-400">การกู้คืนข้อมูลปิดใช้งานในเวอร์ชัน Demo</p>
                        </div>
                    </Modal>
                    <Modal isOpen={showConfirmModal} onClose={() => setShowConfirmModal(false)} title="ยืนยันการนัดหมายสำเร็จ">
                        <div className="text-center space-y-4">
                            ...
                            <div><h4 className="font-bold text-xl text-slate-800">จองคิวเรียบร้อยแล้ว!</h4><p className="text-slate-500 text-sm mt-2">ระบบกำลังส่งข้อความยืนยันไปที่ LINE และ Email</p></div>
                            <button onClick={() => { setShowConfirmModal(false); setView('home'); }} className="w-full bg-teal-600 text-white py-2 rounded-lg hover:bg-teal-700">ตกลง (Close)</button>
                        </div>
                    </Modal>
                    <nav className="bg-white shadow-sm sticky top-0 z-30 border-b border-slate-200">
                        <div className="max-w-5xl mx-auto px-4 h-16 flex items-center justify-between">
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => setView('patient')} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${view === 'patient' ? 'bg-teal-50 text-teal-700' : 'text-slate-500 hover:bg-slate-50'}`}>สำหรับผู้ป่วย</button>
                                <button onClick={handleStaffClick} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${view === 'staff' ? 'bg-teal-50 text-teal-700' : 'text-slate-500 hover:bg-slate-50'}`}>สำหรับเจ้าหน้าที่</button>
                            </div>
                        </div>
                    </nav>
                    <main className="max-w-5xl mx-auto p-4 py-8">
                        {view === 'home' && (
                            <div className="text-center py-12 animate-fade-in">
                                <h1 className="text-4xl font-bold text-teal-900 mb-4">ยินดีต้อนรับสู่ คลินิกสุขภาพดี</h1>
                                <p className="text-slate-600 text-lg mb-8">ระบบนัดหมายออนไลน์ สะดวก รวดเร็ว ปลอดภัย</p>
                                <div className="grid md:grid-cols-2 gap-6 max-w-2xl mx-auto">
                                    <button onClick={() => setView('patient')} className="group bg-white p-8 rounded-2xl shadow-sm hover:shadow-xl border border-slate-200 transition-all hover:-translate-y-1">
                                        ...
                                        <h3 className="text-xl font-bold text-slate-800 mb-2">จองคิวตรวจ</h3><p className="text-slate-500">สำหรับผู้ป่วยที่ต้องการนัดหมายแพทย์ล่วงหน้า</p>
                                    </button>
                                    <button onClick={handleStaffClick} className="group bg-white p-8 rounded-2xl shadow-sm hover:shadow-xl border border-slate-200 transition-all hover:-translate-y-1">
                                        ...
                                        <h3 className="text-xl font-bold text-slate-800 mb-2">เข้าสู่ระบบเจ้าหน้าที่</h3><p className="text-slate-500">สำหรับแพทย์และพนักงานเพื่อจัดการนัดหมาย</p>
                                    </button>
                                </div>
                            </div>
                        )}
